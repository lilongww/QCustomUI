...
ClassifyTreeViewExample::ClassifyTreeViewExample(QWidget* parent)
    : QCtmWindow(parent)
{
    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        for (int j = 0; j < 20; j++)
            root->addChild(new QCtmClassifyTreeIconItem(QIcon(":/ClassifyTreeViewExample/resources/bus-color.svg"), QString("name%1").arg(j)));
        model->addItem(root);
    }
