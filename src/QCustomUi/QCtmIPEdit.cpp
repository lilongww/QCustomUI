	...
	, m_impl(std::make_unique<Impl>())
{
	...
	connect(this, &QWidget::customContextMenuRequested, this, &QCtmIPEdit::onCustomContextMenuRequested);
...
void QCtmIPEdit::setIPAddress(const QString& ip)
{
	auto tmp = ip.split('.');
	if (tmp.size() == 4)
	{
...
QString QCtmIPEdit::ipAddress() const
{
	QStringList ips;
...
QRect QCtmIPEdit::rectOfIpSection(int section) const
{
...
QRect QCtmIPEdit::boundRect(int section, const QRect& rect) const
{
...
bool QCtmIPEdit::setText(QTextLayout& textLayout, const QString& text)
{
	if (!text.isEmpty())
	{
		bool ok{ false };
...
void QCtmIPEdit::keyPressEvent(QKeyEvent* event)
{
...
void QCtmIPEdit::mouseDoubleClickEvent(QMouseEvent* event)
{
...
void QCtmIPEdit::mousePressEvent(QMouseEvent* event)
{
	if (event->button() == Qt::LeftButton && event->modifiers().testFlag(Qt::NoModifier))
	{
		clearSelection();
...
void QCtmIPEdit::mouseMoveEvent(QMouseEvent* event)
{
...
void QCtmIPEdit::mouseReleaseEvent(QMouseEvent* event)
{
	if (event->button() == Qt::LeftButton && event->modifiers().testFlag(Qt::NoModifier))
		m_impl->pressed = false;
}

void QCtmIPEdit::focusInEvent(QFocusEvent* event)
{
...
void QCtmIPEdit::focusOutEvent(QFocusEvent* event)
{
...
int QCtmIPEdit::xToCursor(int x) const
{
	for (int i = 0; i < SECTION_COUNT; i++)
	{
		auto&& rect = rectOfIpSection(i);
...
void QCtmIPEdit::clearSelection()
{
...
void QCtmIPEdit::deleteSelectedText()
{
...
bool QCtmIPEdit::hasSelection() const
{
	for (auto fr : m_impl->selections)
	{
		if (!fr.isEmpty())
...
void QCtmIPEdit::onCustomContextMenuRequested(const QPoint& pos)
{
	QMenu menu(this);
	bool selected = false;
